...
public:
    ...
    void ShortestPath(int start)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            PrintDistance();
            int u = chooseVertex();
            // 최단경로를 모르는 노드 중 현재 발견된 노드에서 가장 가까운 노드를 뽑는다.
            // negative weight가 없으므로 이렇게 뽑은 경로는 당연히 최단경로가 된다.
            found[u] = true;
            for (int w = 0; w < size; w++)
            {
                if (!found[w])
                {
                    ...
                    {
                        // 만약 최단거리를 아직 모르는 w에 대해서
                        // u를 거치는 경로가 기존에 우리가 정해준 경로보다 짧다면
                        // (INF이거나, start부터의 weight이거나 둘 중 하나일 것이다.)
                        // 최단 경로를 u를 거치는 경로로 바꿔준다.
                        // negative weight가 있으면 작동 안함
